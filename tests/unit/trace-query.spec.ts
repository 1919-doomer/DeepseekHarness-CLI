...
describe('M4.4 trace debugger query', () => {
  ...
  it('pages from the newest retained matches in viewport-sized chunks while keeping process-absolute indices', () => {
    ...
    expect(pageOne).toContain('page 1/12 · 170 retained matches')
    expect(pageOne).toContain('0185 unknown root future.185')
    expect(pageOne).toContain('0199 unknown root future.199')
    expect(pageOne).not.toContain('0184 unknown root future.184')

    const pageTwo = renderTraceQuery(context, { mode: 'all', page: 2 })
    expect(pageTwo).toContain('page 2/12')
    expect(pageTwo).toContain('0170 unknown root future.170')
    expect(pageTwo).toContain('0184 unknown root future.184')
    expect(pageTwo).not.toContain('0185 unknown root future.185')

    const pageTwelve = renderTraceQuery(context, { mode: 'all', page: 12 })
    expect(pageTwelve).toContain('0030 unknown root future.30')
    expect(pageTwelve).toContain('0034 unknown root future.34')

    const missing = renderTraceQuery(context, { mode: 'all', page: 13 })
    expect(missing).toContain('No such retained page. Available pages: 1-12.')
  })
})
